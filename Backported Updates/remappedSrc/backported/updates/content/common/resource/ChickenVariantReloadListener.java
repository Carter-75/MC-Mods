...
extends RegistryAwareJsonReloadListener {
    ...
    public void parse(Map<class_2960, JsonElement> resources, class_5455 registryAccess, class_3300 manager, class_3695 profiler) {
        profiler.method_15396("Loading chicken variants");
        ...
        for (Map.Entry<class_2960, JsonElement> entry : resources.entrySet()) {
            ...
            try {
                ChickenVariant.CODEC.parse((DynamicOps)ops, (Object)element).resultOrPartial(error -> VanillaBackport.LOGGER.error("Failed to parse chicken variant {}: {}", (Object)name, error)).ifPresent(variant -> ModBuiltinRegistries.CHICKEN_VARIANTS.registerDataDriven(name, (ChickenVariant)variant));
            }
