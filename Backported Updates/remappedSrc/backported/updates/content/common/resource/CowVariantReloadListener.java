...
extends RegistryAwareJsonReloadListener {
    ...
    public void parse(Map<class_2960, JsonElement> resources, class_5455 registryAccess, class_3300 manager, class_3695 profiler) {
        profiler.method_15396("Loading cow variants");
        ...
        for (Map.Entry<class_2960, JsonElement> entry : resources.entrySet()) {
            ...
            try {
                CowVariant.CODEC.parse((DynamicOps)ops, (Object)element).resultOrPartial(error -> VanillaBackport.LOGGER.error("Failed to parse cow variant {}: {}", (Object)name, error)).ifPresent(variant -> ModBuiltinRegistries.COW_VARIANTS.registerDataDriven(name, (CowVariant)variant));
            }
            catch (JsonParseException exception) {
                VanillaBackport.LOGGER.error("Failed to parse cow variant JSON {}: {}", new Object[]{name, exception.getMessage(), exception});
            }
            catch (Exception exception) {
                VanillaBackport.LOGGER.error("Unexpected error processing cow variant {}", (Object)name, (Object)exception);
            }
        }
