...
public class CatVariantReloadListener
extends RegistryAwareJsonReloadListener {
    ...
    public void parse(Map<class_2960, JsonElement> resources, class_5455 registryAccess, class_3300 manager, class_3695 profiler) {
        profiler.method_15396("Loading cat variants");
        ...
        for (Map.Entry<class_2960, JsonElement> entry : resources.entrySet()) {
            ...
            try {
                FrogDataVariant.CODEC.parse((DynamicOps)ops, (Object)element).resultOrPartial(error -> VanillaBackport.LOGGER.error("Failed to parse cat variant {}: {}", (Object)name, error)).ifPresent(variant -> ModBuiltinRegistries.FROG_VARIANTS.registerDataDriven(name, (FrogDataVariant)variant));
            }
            catch (JsonParseException exception) {
                VanillaBackport.LOGGER.error("Failed to parse cat variant JSON {}: {}", new Object[]{name, exception.getMessage(), exception});
            }
            catch (Exception exception) {
                VanillaBackport.LOGGER.error("Unexpected error processing cat variant {}", (Object)name, (Object)exception);
            }
        }
