...
extends RegistryAwareJsonReloadListener {
    ...
    public void parse(Map<class_2960, JsonElement> resources, class_5455 registryAccess, class_3300 manager, class_3695 profiler) {
        ...
        for (Map.Entry<class_2960, JsonElement> entry : resources.entrySet()) {
            ...
            try {
                WolfDataVariant.CODEC.parse((DynamicOps)ops, (Object)element).resultOrPartial(error -> VanillaBackport.LOGGER.error("Failed to parse wolf variant {}: {}", (Object)name, error)).ifPresent(variant -> ModBuiltinRegistries.WOLF_VARIANTS.registerDataDriven(name, (WolfDataVariant)variant));
            }
