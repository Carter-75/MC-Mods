...
implements Control<Integer> {
    ...
    public static interface ValueFormatter {
        public Text format(int var1);

        public static ValueFormatter realMinutes() {
            return value -> {
                float minutes = (float)value / 1200.0f;
                if (minutes == (float)((int)minutes)) {
                    return Text.translatable((String)"config.copperagebackport.minutes", (Object[])new Object[]{(int)minutes});
                }
                return Text.translatable((String)"config.copperagebackport.minutes_decimal", (Object[])new Object[]{String.format("%.1f", Float.valueOf(minutes))});
            };
        }

        public static ValueFormatter percentage() {
            return value -> Text.literal((String)(value + "%"));
        }

        public static ValueFormatter withSuffix(String suffix) {
            return value -> Text.literal((String)(value + " " + suffix));
        }

        public static ValueFormatter number() {
            return value -> Text.literal((String)String.valueOf(value));
        }
    }
    ...
    extends ControlElement<Integer> {
        ...
        protected void renderControl(DrawContext graphics, int mouseX, int mouseY, float partialTick) {
            ...
            if (this.editing && this.selectionStart != this.selectionEnd) {
                ...
                int selX1 = textX + this.font.getWidth(beforeSel);
                int selX2 = selX1 + this.font.getWidth(selText);
            ...
            if (this.editing) {
                ...
                int cursorX = textX + this.font.getWidth(beforeCursor);
                this.drawRect(graphics, cursorX, textY - 1, cursorX + 1, textY + 9, -1);
            }
            if (this.formatter != null && !this.editing && !this.hasError) {
                Text hint = this.formatter.format((Integer)this.option.getValue());
                hintX = fieldX - 8 - this.getStringWidth((StringVisitable)hint);
                this.drawString(graphics, hint, hintX, textY, -7829368);
            }
            if (this.hasError) {
                MutableText errorHint = Text.literal((String)("Min: " + effectiveMin));
                hintX = fieldX - 8 - this.getStringWidth((StringVisitable)errorHint);
                this.drawString(graphics, (Text)errorHint, hintX, textY, -48060);
            }
        }
        ...
        private int getCharIndexAtX(int x) {
            ...
            int idx;
            int width = 0;
            for (idx = 0; idx < this.text.length() && width + (charWidth = this.font.getWidth(String.valueOf(this.text.charAt(idx)))) / 2 <= x; ++idx) {
                width += charWidth;
            }
            return idx;
        }
        ...
        private void finishEditing() {
            ...
            try {
                int value = Integer.parseInt(this.text);
                int effectiveMin = this.getEffectiveMin();
                value = MathHelper.clamp((int)value, (int)effectiveMin, (int)this.maxValue);
