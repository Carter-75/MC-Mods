...
public abstract class GenericBuilder<Base extends Config, Result extends FileConfig> {
    ...
    public Result build() {
        ...
        if (this.sync) {
            fileConfig = (FileConfig)new WriteSyncFileConfig<Base>(this.getConfig(), this.file, this.charset, this.writer, this.writingMode, this.parser, this.parsingMode, this.nefAction);
        } else {
            if (this.autoreload) {
                this.concurrent();
            }
            fileConfig = (FileConfig)new WriteAsyncFileConfig<Base>(this.getConfig(), this.file, this.charset, this.writer, this.writingMode, this.parser, this.parsingMode, this.nefAction);
        }
        if (this.autoreload) {
