...
import backported.updates.foundation.core.util.event.SimpleEvent;
...
public abstract class Event<T> {
    ...
    public static <T> Event<T> create(Class<? super T> type) {
        return Event.create(type, callbacks -> (T) Proxy.newProxyInstance(Event.class.getClassLoader(), new Class[]{type}, (proxy, method, args) -> {
    ...
    public static <T> Event<T> cancellable(Class<? super T> type) {
        return Event.create(type, callbacks -> (T) Proxy.newProxyInstance(Event.class.getClassLoader(), new Class[]{type}, (proxy, method, args) -> {
            for (Object callback : callbacks) {
                CancellableResult result = Objects.requireNonNull((CancellableResult)(((Event.invokeFast(callback, method, args)))));
