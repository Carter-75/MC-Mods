...
extends RegistryAwareJsonReloadListener {
    ...
    public void parse(Map<Identifier, JsonElement> resources, DynamicRegistryManager registryAccess, ResourceManager manager, Profiler profiler) {
        ...
        for (Map.Entry<Identifier, JsonElement> entry : resources.entrySet()) {
            ...
            try {
                FrogDataVariant.CODEC.parse((DynamicOps)ops, element).resultOrPartial(error -> VanillaBackport.LOGGER.error("Failed to parse frog variant {}: {}", name, error)).ifPresent(variant -> ModBuiltinRegistries.FROG_VARIANTS.registerDataDriven(name, (FrogDataVariant)variant));
            }
