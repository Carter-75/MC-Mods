...
extends RegistryAwareJsonReloadListener {
    ...
    public void parse(Map<Identifier, JsonElement> resources, DynamicRegistryManager registryAccess, ResourceManager manager, Profiler profiler) {
        ...
        for (Map.Entry<Identifier, JsonElement> entry : resources.entrySet()) {
            ...
            try {
                ChickenVariant.CODEC.parse((DynamicOps)ops, element).resultOrPartial(error -> VanillaBackport.LOGGER.error("Failed to parse chicken variant {}: {}", name, error)).ifPresent(variant -> ModBuiltinRegistries.CHICKEN_VARIANTS.registerDataDriven(name, (ChickenVariant)variant));
            }
