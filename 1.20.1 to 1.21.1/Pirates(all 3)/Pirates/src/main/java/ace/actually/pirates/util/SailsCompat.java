...
public class SailsCompat {
    ...
    public static void moveTowards(MotionInvokingBlockEntity be, SeatedControllingPlayer power, LoadedServerShip ship) {
        ...
        if (be.getLdx() == -1.0) {
            be.setLdx(SailsCompat.vdis(be.getTarget()[0], v3d.x()));
            be.setLdz(SailsCompat.vdis(be.getTarget()[2], v3d.z()));
        } else {
            double cdx = SailsCompat.vdis(be.getTarget()[0], v3d.x());
            double cdz = SailsCompat.vdis(be.getTarget()[2], v3d.z());
