...
public class Random {
    ...
    private static final class Xorshiro128 {
        ...
        private static long rotl_JDK4(long x, int k2) {
            return x << k2 | x >>> 64 - k2;
        }

        private static long rotl_JDK5(long x, int k2) {
            return Long.rotateLeft(x, k2);
        }

        private static long rotl(long x, int k2) {
            if (Runtime.HAS_Long_rotateLeft) {
                return Xorshiro128.rotl_JDK5(x, k2);
            }
            return Xorshiro128.rotl_JDK4(x, k2);
        }
