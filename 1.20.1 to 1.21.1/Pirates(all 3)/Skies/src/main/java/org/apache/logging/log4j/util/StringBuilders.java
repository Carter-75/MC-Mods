...
public final class StringBuilders {
    ...
    public static void escapeJson(StringBuilder toAppendTo, int start) {
        int escapeCount = 0;
        block11: for (int i2 = start; i2 < toAppendTo.length(); ++i2) {
            ...
            switch (c2) {
                ...
                default: {
                    if (!Character.isISOControl(c2)) continue block11;
                    escapeCount += 5;
                }
            }
        }
        ...
        block12: for (int i3 = lastChar; lastPos > i3; --i3) {
            char c3 = toAppendTo.charAt(i3);
            switch (c3) {
                case '\b': {
                ...
                case '\"': 
                case '\\': {
                    lastPos = StringBuilders.escapeAndDecrement(toAppendTo, lastPos, c3);
                    continue block12;
                }
                default: {
                    if (Character.isISOControl(c3)) {
                        toAppendTo.setCharAt(lastPos--, Chars.getUpperCaseHex(c3 & 0xF));
                        toAppendTo.setCharAt(lastPos--, Chars.getUpperCaseHex((c3 & 0xF0) >> 4));
                    ...
                    toAppendTo.setCharAt(lastPos, c3);
                    --lastPos;
                }
            }
        }
    ...
    private static int escapeAndDecrement(StringBuilder toAppendTo, int lastPos, char c2) {
    ...
    public static void escapeXml(StringBuilder toAppendTo, int start) {
        int escapeCount = 0;
        block12: for (int i2 = start; i2 < toAppendTo.length(); ++i2) {
            char c2 = toAppendTo.charAt(i2);
            switch (c2) {
                case '&': {
                    escapeCount += 4;
        ...
        toAppendTo.setLength(toAppendTo.length() + escapeCount);
        int lastPos = toAppendTo.length() - 1;
        block13: for (int i3 = lastChar; lastPos > i3; --i3) {
            char c3 = toAppendTo.charAt(i3);
            switch (c3) {
                case '&': {
                    toAppendTo.setCharAt(lastPos--, ';');
