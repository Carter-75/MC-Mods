...
Clearable {
    ...
    public Object[] swapParameters(Object[] emptyReplacement) {
        ...
        if (this.varargs == null) {
            ...
            if (emptyReplacement.length >= 10) {
                ...
                result2 = emptyReplacement;
            } else {
                this.params = new Object[10];
            }
        } else {
            result2 = this.argCount <= emptyReplacement.length ? emptyReplacement : new Object[this.argCount];
            System.arraycopy(this.varargs, 0, result2, 0, this.argCount);
        }
        return result2;
    }
    ...
    public <S> void forEachParameter(ParameterConsumer<S> action, S state) {
        ...
        for (int i2 = 0; i2 < this.argCount; i2 = (int)((short)(i2 + 1))) {
            action.accept(parameters[i2], i2, state);
        }
    }
    ...
    private StringBuilder getBuffer() {
        ...
        if ((result2 = this.buffer.get()) == null) {
            int currentPatternLength = this.messagePattern == null ? 0 : this.messagePattern.length();
            result2 = new StringBuilder(Math.max(512, currentPatternLength * 2));
            this.buffer.set(result2);
        }
        result2.setLength(0);
        return result2;
    }
