...
public final class ThreadContextMapFactory {
    ...
    public static ThreadContextMap createThreadContextMap() {
        ...
        ThreadContextMap result2 = null;
        if (ThreadContextMapName != null) {
            try {
                Class<?> clazz = cl.loadClass(ThreadContextMapName);
                if (ThreadContextMap.class.isAssignableFrom(clazz)) {
                    result2 = (ThreadContextMap)clazz.newInstance();
                }
            }
        ...
        if (result2 == null && ProviderUtil.hasProviders() && LogManager.getFactory() != null) {
            ...
            for (Provider provider : ProviderUtil.getProviders()) {
                ...
                try {
                    result2 = clazz.newInstance();
                    break;
                }
                catch (Exception e2) {
                    LOGGER.error("Unable to locate or load configured ThreadContextMap {}", (Object)provider.getThreadContextMap(), (Object)e2);
                    result2 = ThreadContextMapFactory.createDefaultThreadContextMap();
                }
            }
        }
        if (result2 == null) {
