...
Serializable {
    ...
    private static MessageFactory2 createDefaultMessageFactory() {
        try {
            MessageFactory result2 = DEFAULT_MESSAGE_FACTORY_CLASS.newInstance();
            return AbstractLogger.narrow(result2);
        }
    ...
    private static MessageFactory2 narrow(MessageFactory result2) {
        if (result2 instanceof MessageFactory2) {
            return (MessageFactory2)result2;
        }
        return new MessageFactory2Adapter(result2);
    }
    ...
    protected EntryMessage entryMsg(String format, Object ... params) {
        int count2;
        int n2 = count2 = params == null ? 0 : params.length;
        if (count2 == 0) {
        ...
        for (int i2 = 0; i2 < count2; ++i2) {
            ...
            sb.append((parm = params[i2]) instanceof Message ? ((Message)parm).getFormattedMessage() : String.valueOf(parm));
        }
        sb.append(')');
    ...
    protected EntryMessage entryMsg(String format, MessageSupplier ... paramSuppliers) {
        int count2 = paramSuppliers == null ? 0 : paramSuppliers.length;
        Object[] params = new Object[count2];
        for (int i2 = 0; i2 < count2; ++i2) {
            params[i2] = paramSuppliers[i2].get();
            params[i2] = params[i2] != null ? ((Message)params[i2]).getFormattedMessage() : null;
        }
    ...
    protected EntryMessage entryMsg(String format, Supplier<?> ... paramSuppliers) {
        ...
        for (int i2 = 0; i2 < count2; ++i2) {
            params[i2] = paramSuppliers[i2].get();
            if (!(params[i2] instanceof Message)) continue;
            params[i2] = ((Message)params[i2]).getFormattedMessage();
        }
        return this.entryMsg(format, params);
    }
    ...
    protected <R> R exit(String fqcn, R result2) {
        if (this.isEnabled(Level.TRACE, EXIT_MARKER, (CharSequence)null, null)) {
            this.logMessageSafely(fqcn, Level.TRACE, EXIT_MARKER, this.exitMsg(null, result2), null);
        }
        return result2;
    }

    protected <R> R exit(String fqcn, String format, R result2) {
        if (this.isEnabled(Level.TRACE, EXIT_MARKER, (CharSequence)null, null)) {
            this.logMessageSafely(fqcn, Level.TRACE, EXIT_MARKER, this.exitMsg(format, result2), null);
        }
        return result2;
    }

    protected Message exitMsg(String format, Object result2) {
        if (result2 == null) {
            if (format == null) {
        ...
        return this.messageFactory.newMessage("Exit: " + format, result2);
    }
    ...
    private static int[] getRecursionDepthHolder() {
        int[] result2 = recursionDepthHolder.get();
        if (result2 == null) {
            result2 = new int[1];
    ...
    @Override
    public <R> R traceExit(R result2) {
        return this.exit(FQCN, null, result2);
    }

    @Override
    public <R> R traceExit(String format, R result2) {
        return this.exit(FQCN, format, result2);
    }
    ...
    @Override
    public <R> R traceExit(EntryMessage message, R result2) {
        if (message != null && this.isEnabled(Level.TRACE, EXIT_MARKER, message, null)) {
            this.logMessageSafely(FQCN, Level.TRACE, EXIT_MARKER, this.flowMessageFactory.newExitMessage(result2, message), null);
        }
        return result2;
    }

    @Override
    public <R> R traceExit(Message message, R result2) {
        if (message != null && this.isEnabled(Level.TRACE, EXIT_MARKER, message, null)) {
            this.logMessageSafely(FQCN, Level.TRACE, EXIT_MARKER, this.flowMessageFactory.newExitMessage(result2, message), null);
        }
        return result2;
    ...
    private void readObject(ObjectInputStream s) throws ClassNotFoundException, IOException {
        s.defaultReadObject();
        try {
            Field f2 = this.getClass().getDeclaredField("logBuilder");
            f2.setAccessible(true);
            f2.set(this, new LocalLogBuilder(this));
        }
