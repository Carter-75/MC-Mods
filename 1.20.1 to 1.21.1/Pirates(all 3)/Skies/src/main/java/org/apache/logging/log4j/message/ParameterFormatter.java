...
final class ParameterFormatter {
    ...
    static int countArgumentPlaceholders(String messagePattern) {
        ...
        for (int i2 = 0; i2 < length - 1; ++i2) {
            ...
            if (curChar == '{') {
                if (!isEscaped && messagePattern.charAt(i2 + 1) == '}') {
                    ++result2;
                    ++i2;
                }
                isEscaped = false;
    ...
    static int countArgumentPlaceholders3(char[] messagePattern, int length, int[] indices) {
        ...
        boolean isEscaped = false;
        for (int i2 = 0; i2 < length - 1; ++i2) {
            char curChar = messagePattern[i2];
            if (curChar == '\\') {
                isEscaped = !isEscaped;
                continue;
            }
            if (curChar == '{') {
                if (!isEscaped && messagePattern[i2 + 1] == '}') {
                    indices[result2] = i2++;
                    ++result2;
                }
                isEscaped = false;
    ...
    static String format(String messagePattern, Object[] arguments) {
        ...
        ParameterFormatter.formatMessage(result2, messagePattern, arguments, argCount);
    ...
    static void formatMessage2(StringBuilder buffer, String messagePattern, Object[] arguments, int argCount, int[] indices) {
        ...
        for (int i2 = 0; i2 < argCount; ++i2) {
            buffer.append(messagePattern, previous, indices[i2]);
            previous = indices[i2] + 2;
            ParameterFormatter.recursiveDeepToString(arguments[i2], buffer);
        }
    ...
    static void formatMessage(StringBuilder buffer, String messagePattern, Object[] arguments, int argCount) {
        ...
        for (i2 = 0; i2 < len - 1; ++i2) {
            ...
            if (ParameterFormatter.isDelimPair(curChar, messagePattern, i2)) {
                ++i2;
        ...
        ParameterFormatter.handleRemainingCharIfAny(messagePattern, len, buffer, escapeCounter, i2);
    }
    ...
    private static void handleRemainingCharIfAny(String messagePattern, int len, StringBuilder buffer, int escapeCounter, int i2) {
        if (i2 == len - 1) {
            char curChar = messagePattern.charAt(i2);
