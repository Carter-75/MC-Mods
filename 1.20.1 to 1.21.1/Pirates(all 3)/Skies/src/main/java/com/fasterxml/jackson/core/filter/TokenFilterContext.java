...
extends JsonStreamContext {
    ...
    protected TokenFilterContext(int type, TokenFilterContext parent, TokenFilter filter2, boolean startHandled) {
        this._type = type;
        this._parent = parent;
    ...
    protected TokenFilterContext reset(int type, TokenFilter filter2, boolean startWritten) {
        this._type = type;
        this._filter = filter2;
        this._index = -1;
    ...
    public static TokenFilterContext createRootContext(TokenFilter filter2) {
        return new TokenFilterContext(0, null, filter2, true);
    }

    public TokenFilterContext createChildArrayContext(TokenFilter filter2, boolean writeStart) {
        TokenFilterContext ctxt = this._child;
        if (ctxt == null) {
            this._child = ctxt = new TokenFilterContext(1, this, filter2, writeStart);
            return ctxt;
        }
        return ctxt.reset(1, filter2, writeStart);
    }

    public TokenFilterContext createChildObjectContext(TokenFilter filter2, boolean writeStart) {
        TokenFilterContext ctxt = this._child;
        if (ctxt == null) {
            this._child = ctxt = new TokenFilterContext(2, this, filter2, writeStart);
            return ctxt;
        }
        return ctxt.reset(2, filter2, writeStart);
    }
    ...
    public TokenFilter checkValue(TokenFilter filter2) {
        if (this._type == 2) {
            return filter2;
        }
        int ix = ++this._index;
        if (this._type == 1) {
            return filter2.includeElement(ix);
        }
        return filter2.includeRootValue(ix);
    }
