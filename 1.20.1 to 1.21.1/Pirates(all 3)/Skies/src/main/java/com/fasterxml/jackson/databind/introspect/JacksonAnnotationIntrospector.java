...
implements Serializable {
    ...
    public String findEnumValue(Enum<?> value) {
        try {
            String n2;
            JsonProperty prop;
            Field f2 = value.getDeclaringClass().getField(value.name());
    ...
    @Override
    public JsonIgnoreProperties.Value findPropertyIgnoralByName(MapperConfig<?> config, Annotated a2) {
    ...
    public JacksonInject.Value findInjectableValue(AnnotatedMember m2) {
        ...
        if (!v.hasId()) {
            AnnotatedMethod am;
            String id = !(m2 instanceof AnnotatedMethod) ? m2.getRawType().getName() : ((am = (AnnotatedMethod)m2).getParameterCount() == 0 ? m2.getRawType().getName() : am.getRawParameterType(0).getName());
            v = v.withId(id);
        }
        return v;
    }
    ...
    public Object findInjectableValueId(AnnotatedMember m2) {
        JacksonInject.Value v = this.findInjectableValue(m2);
    ...
    public List<NamedType> findSubtypes(Annotated a2) {
        JsonSubTypes t = this._findAnnotation(a2, JsonSubTypes.class);
        if (t == null) {
            return null;
        }
            ...
            if (serClass != null) {
                if (type.hasRawClass(serClass)) {
                    ...
                    try {
                        ...
                        throw this._databindException(iae, String.format("Failed to widen type %s with annotation (value %s), from '%s': %s", type, serClass.getName(), a2.getName(), iae.getMessage()));
                    }
                }
            }
        }
                ...
                block27: {
                    if (keyType.hasRawClass(keyClass)) {
                        ...
                        try {
                            ...
                            throw this._databindException(iae, String.format("Failed to widen key type of %s with concrete-type annotation (value %s), from '%s': %s", type, keyClass.getName(), a2.getName(), iae.getMessage()));
                        }
                    }
                }
                ...
                block28: {
                    if (contentType.hasRawClass(contentClass)) {
                        ...
                        try {
                            ...
                            throw this._databindException(iae, String.format("Internal error: failed to refine value type of %s with concrete-type annotation (value %s), from '%s': %s", type, contentClass.getName(), a2.getName(), iae.getMessage()));
                        }
                    }
                }
    ...
    public void findAndAddVirtualProperties(MapperConfig<?> config, AnnotatedClass ac, List<BeanPropertyWriter> properties) {
        ...
        for (int i2 = 0; i2 < len; ++i2) {
            ...
            BeanPropertyWriter bpw = this._constructVirtualProperty(attrs[i2], config, ac, propType);
            if (prepend) {
        ...
        for (int i3 = 0; i3 < len2; ++i3) {
            BeanPropertyWriter bpw = this._constructVirtualProperty(props[i3], config, ac);
            if (prepend) {
    ...
    public JavaType refineDeserializationType(MapperConfig<?> config, Annotated a2, JavaType baseType) throws JsonMappingException {
        ...
        if (valueClass != null && !type.hasRawClass(valueClass) && !this._primitiveAndWrapper(type, valueClass)) {
            try {
                ...
                throw this._databindException(iae, String.format("Failed to narrow type %s with annotation (value %s), from '%s': %s", type, valueClass.getName(), a2.getName(), iae.getMessage()));
            }
        }
        if (type.isMapLikeType()) {
            Class<?> keyClass;
            ...
            if (keyClass != null && !this._primitiveAndWrapper(keyType, keyClass)) {
                try {
                    ...
                    throw this._databindException(iae, String.format("Failed to narrow key type of %s with concrete-type annotation (value %s), from '%s': %s", type, keyClass.getName(), a2.getName(), iae.getMessage()));
                }
            }
        }
        if ((contentType = type.getContentType()) != null) {
            ...
            if (contentClass != null && !this._primitiveAndWrapper(contentType, contentClass)) {
                try {
                    ...
                    throw this._databindException(iae, String.format("Failed to narrow value type of %s with concrete-type annotation (value %s), from '%s': %s", type, contentClass.getName(), a2.getName(), iae.getMessage()));
                }
            }
        }
        return type;
