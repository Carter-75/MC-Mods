...
extends ObjectMapper {
    ...
    public CBORMapper(CBORFactory f2) {
        super(f2);
    }
    ...
    extends MapperBuilder<CBORMapper, Builder> {
        ...
        public Builder(CBORMapper m2) {
            super(m2);
            this._streamFactory = m2.getFactory();
        }

        public Builder enable(CBORGenerator.Feature ... features) {
            for (CBORGenerator.Feature f2 : features) {
                this._streamFactory.enable(f2);
            }
            return this;
        }
        ...
        public Builder disable(CBORGenerator.Feature ... features) {
            for (CBORGenerator.Feature f2 : features) {
                this._streamFactory.disable(f2);
            }
            return this;
        }

        public Builder configure(CBORGenerator.Feature f2, boolean state) {
            if (state) {
                this._streamFactory.enable(f2);
            } else {
                this._streamFactory.disable(f2);
