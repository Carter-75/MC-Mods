...
SchemaAware {
    ...
    private static final BeanPropertyWriter[] rename(BeanPropertyWriter[] props, NameTransformer transformer) {
        ...
        for (int i2 = 0; i2 < len; ++i2) {
            ...
            result2[i2] = bpw.rename(transformer);
        }
        return result2;
    }
            ...
            if (objectIdInfo == null) {
                ...
                if (implClass == ObjectIdGenerators.PropertyGenerator.class) {
                    ...
                    while (true) {
                        ...
                        if (propName.equals((prop = this._props[i2]).getName())) break;
                        ++i2;
                    }
                    idProp = prop;
                    idPropOrigIndex = i2;
    ...
    protected void _serializeObjectId(Object bean, JsonGenerator g2, SerializerProvider provider, TypeSerializer typeSer, WritableObjectId objectId) throws IOException {
        ...
        g2.setCurrentValue(bean);
        objectId.writeAsField(g2, provider, w);
        if (this._propertyFilterId != null) {
    ...
    protected void serializeFieldsFiltered(Object bean, JsonGenerator gen, SerializerProvider provider) throws IOException {
        int i2;
        BeanPropertyWriter[] props = this._filteredProps != null && provider.getActiveView() != null ? this._filteredProps : this._props;
        PropertyFilter filter2 = this.findPropertyFilter(provider, this._propertyFilterId, bean);
        if (filter2 == null) {
            this.serializeFields(bean, gen, provider);
            return;
        }
        try {
            for (BeanPropertyWriter prop : props) {
                if (prop == null) continue;
                filter2.serializeAsField(bean, gen, provider, prop);
            }
            if (this._anyGetterWriter != null) {
                this._anyGetterWriter.getAndFilter(bean, gen, provider, filter2);
            }
        }
        catch (Exception e2) {
            String name = i2 == props.length ? "[anySetter]" : props[i2].getName();
    ...
    public JsonNode getSchema(SerializerProvider provider, Type typeHint) throws JsonMappingException {
        ...
        PropertyFilter filter2 = this._propertyFilterId != null ? this.findPropertyFilter(provider, this._propertyFilterId, null) : null;
        for (int i2 = 0; i2 < this._props.length; ++i2) {
            ...
            filter2.depositSchemaProperty((PropertyWriter)prop, propertiesNode, provider);
        }
    ...
    public void acceptJsonFormatVisitor(JsonFormatVisitorWrapper visitor, JavaType typeHint) throws JsonMappingException {
        ...
        if (this._propertyFilterId != null) {
            PropertyFilter filter2 = this.findPropertyFilter(visitor.getProvider(), this._propertyFilterId, null);
            int end = this._props.length;
            for (int i2 = 0; i2 < end; ++i2) {
                filter2.depositSchemaProperty((PropertyWriter)this._props[i2], objectVisitor, provider);
            }
        } else {
