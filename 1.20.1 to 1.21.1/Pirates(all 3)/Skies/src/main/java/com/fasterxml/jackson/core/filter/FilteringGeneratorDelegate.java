...
extends JsonGeneratorDelegate {
    ...
    public FilteringGeneratorDelegate(JsonGenerator d2, TokenFilter f2, boolean includePath, boolean allowMultipleMatches) {
        this(d2, f2, includePath ? TokenFilter.Inclusion.INCLUDE_ALL_AND_PATH : TokenFilter.Inclusion.ONLY_INCLUDE_ALL, allowMultipleMatches);
    }

    public FilteringGeneratorDelegate(JsonGenerator d2, TokenFilter f2, TokenFilter.Inclusion inclusion, boolean allowMultipleMatches) {
        super(d2, false);
        this.rootFilter = f2;
        this._itemFilter = f2;
    ...
    public void writeStartObject() throws IOException {
        ...
        if (f2 == TokenFilter.INCLUDE_ALL) {
            ...
            this.delegate.writeStartObject();
        } else if (f2 != null && this._inclusion == TokenFilter.Inclusion.INCLUDE_NON_NULL) {
            this._checkParentPath(false);
            this._filterContext = this._filterContext.createChildObjectContext(f2, true);
            this.delegate.writeStartObject();
        } else {
