...
extends StdScalarDeserializer<T> {
    ...
    extends FromStringDeserializer<Object> {
        ...
        private Locale _deserializeLocale(String value, DeserializationContext ctxt) throws IOException {
            ...
            String first2 = value.substring(0, ix);
            if ((ix = this._firstHyphenOrUnderscore(value = value.substring(ix + 1))) < 0) {
                return new Locale(first2, value);
            }
            String second2 = value.substring(0, ix);
            int extMarkerIx = value.indexOf(LOCALE_EXT_MARKER);
            if (extMarkerIx < 0) {
                return new Locale(first2, second2, value.substring(ix + 1));
            }
            return this._deSerializeBCP47Locale(value, ix, first2, second2, extMarkerIx);
        }

        private Locale _deSerializeBCP47Locale(String value, int ix, String first2, String second2, int extMarkerIx) {
            String third = "";
            try {
                int underscoreIx;
                ...
                if ((underscoreIx = (value = value.substring(extMarkerIx + 2)).indexOf(95)) < 0) {
                    ...
                    if (hyphenIx < 0) {
                        return new Locale.Builder().setLanguage(first2).setRegion(second2).setVariant(third).setScript(value).build();
                    }
                    return new Locale.Builder().setLanguage(first2).setRegion(second2).setVariant(third).setExtension(value.charAt(0), value.substring(hyphenIx + 1)).build();
                }
                int len = value.length();
                Locale.Builder b2 = new Locale.Builder().setLanguage(first2).setRegion(second2).setVariant(third).setScript(value.substring(0, underscoreIx));
                if (underscoreIx + 1 < len) {
                    b2 = b2.setExtension(value.charAt(underscoreIx + 1), value.substring(Math.min(len, underscoreIx + 3)));
                }
                return b2.build();
            }
            catch (IllformedLocaleException ex) {
                return new Locale(first2, second2, third);
            }
        }
    }
}
