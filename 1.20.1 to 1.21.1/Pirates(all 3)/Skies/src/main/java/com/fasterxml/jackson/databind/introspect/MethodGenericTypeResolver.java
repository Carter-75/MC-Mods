...
final class MethodGenericTypeResolver {
    ...
    static TypeBindings bindMethodTypeParameters(Method candidate, JavaType requestedType, TypeResolutionContext emptyTypeResCtxt) {
        ...
        for (int i2 = 0; i2 < methodReturnTypeArguments.length; ++i2) {
            Type methodReturnTypeArgument = methodReturnTypeArguments[i2];
            ...
            JavaType bindTarget = requestedType.getBindings().getBoundType(i2);
            if (bindTarget == null) {
                return null;
            }
    ...
    private static boolean pessimisticallyValidateBound(TypeResolutionContext context, JavaType boundType, Type type) {
        ...
        if (parameterized != null && Objects.equals(boundType.getRawClass(), parameterized.getRawType())) {
            ...
            for (int i2 = 0; i2 < bindings.size(); ++i2) {
                Type typeArg;
                JavaType boundTypeBound = bindings.getBoundType(i2);
                if (MethodGenericTypeResolver.pessimisticallyValidateBound(context, boundTypeBound, typeArg = typeArguments[i2])) continue;
                return false;
            }
        }
        return true;
