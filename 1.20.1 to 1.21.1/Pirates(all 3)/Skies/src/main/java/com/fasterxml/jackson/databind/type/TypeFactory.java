...
implements Serializable {
    ...
    @Deprecated
    public TypeFactory withCache(LRUMap<Object, JavaType> cache2) {
        return new TypeFactory(cache2, this._parser, this._modifiers, this._classLoader);
    }

    public TypeFactory withCache(LookupCache<Object, JavaType> cache2) {
        return new TypeFactory(cache2, this._parser, this._modifiers, this._classLoader);
    }
...
lbl22:
    ...
    private TypeBindings _bindingsForSubtype(JavaType baseType, int typeParamCount, Class<?> subclass, boolean relaxedCompatibilityCheck) {
        ...
        for (int i3 = 0; i3 < typeParamCount; ++i3) {
            JavaType t = ((PlaceholderForType)placeholders[i3]).actualType();
            if (t == null) {
    ...
    private String _resolveTypePlaceholders(JavaType sourceType, JavaType actualType) throws IllegalArgumentException {
        ...
        for (int i2 = 0; i2 < expCount; ++i2) {
            JavaType act;
            JavaType exp = expectedTypes.get(i2);
            JavaType javaType = act = i2 < actCount ? actualTypes.get(i2) : TypeFactory.unknownType();
            if (this._verifyAndResolvePlaceholders(exp, act) || exp.hasRawClass(Object.class) || i2 == 0 && sourceType.isMapLikeType() && act.hasRawClass(Object.class) || exp.isInterface() && exp.isTypeOrSuperTypeOf(act.getRawClass())) continue;
            return String.format("Type parameter #%d/%d differs; can not specialize %s with %s", i2 + 1, expCount, exp.toCanonical(), act.toCanonical());
        }
        return null;
    }
    ...
    private boolean _verifyAndResolvePlaceholders(JavaType exp, JavaType act) {
        ...
        for (int i2 = 0; i2 < len; ++i2) {
            ...
            if (this._verifyAndResolvePlaceholders(exp2, act2 = actualTypes.get(i2))) continue;
            return false;
        }
        return true;
    }
    ...
    public JavaType constructParametricType(Class<?> parametrized, Class<?> ... parameterClasses) {
        ...
        for (int i2 = 0; i2 < len; ++i2) {
            pt[i2] = this._fromClass(null, parameterClasses[i2], EMPTY_BINDINGS);
        }
    ...
    protected JavaType _applyModifiers(Type srcType, JavaType resolvedType) {
        ...
        for (TypeModifier mod : this._modifiers) {
            JavaType t = mod.modifyType(resultType, srcType, b2, this);
            if (t == null) {
    ...
    protected JavaType _fromClass(ClassStack context, Class<?> rawType, TypeBindings bindings) {
        ...
        result2 = this._typeCache.get(key);
        if (result2 != null) {
            return result2;
        }
        if (context == null) {
            ...
            ClassStack prev2 = context.find(rawType);
            if (prev2 != null) {
                ResolvedRecursiveType selfRef = new ResolvedRecursiveType(rawType, EMPTY_BINDINGS);
                prev2.addSelfReference(selfRef);
                return selfRef;
            }
    ...
    protected JavaType _fromWellKnownInterface(ClassStack context, Class<?> rawType, TypeBindings bindings, JavaType superClass, JavaType[] superInterfaces) {
        ...
        for (int i2 = 0; i2 < intCount; ++i2) {
            JavaType result2 = superInterfaces[i2].refine(rawType, bindings, superClass, superInterfaces);
