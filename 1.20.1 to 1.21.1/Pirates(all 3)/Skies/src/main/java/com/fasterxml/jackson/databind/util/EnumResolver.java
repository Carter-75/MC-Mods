...
implements Serializable {
    ...
    protected EnumResolver(Class<Enum<?>> enumClass, Enum<?>[] enums, HashMap<String, Enum<?>> map2, Enum<?> defaultValue, boolean isIgnoreCase, boolean isFromIntValue) {
    ...
    @Deprecated
    protected EnumResolver(Class<Enum<?>> enumClass, Enum<?>[] enums, HashMap<String, Enum<?>> map2, Enum<?> defaultValue, boolean isIgnoreCase) {
        this(enumClass, enums, map2, defaultValue, isIgnoreCase, false);
    }
    ...
    protected static EnumResolver _constructFor(Class<?> enumCls0, AnnotationIntrospector ai, boolean isIgnoreCase) {
        ...
        for (int i2 = 0; i2 < len; ++i2) {
            ...
            map2.put(name, enumValue);
            String[] aliases = allAliases[i2];
            if (aliases == null) continue;
            for (String alias : aliases) {
                if (map2.containsKey(alias)) continue;
                map2.put(alias, enumValue);
            }
        }
        return new EnumResolver(enumCls, enumConstants, map2, EnumResolver._enumDefault(ai, enumCls), isIgnoreCase, false);
    }
    ...
    protected static EnumResolver _constructUsingToString(Class<?> enumCls0, AnnotationIntrospector ai, boolean isIgnoreCase) {
        ...
        while (--i2 >= 0) {
            Enum<?> enumValue = enumConstants[i2];
            map2.put(enumValue.toString(), enumValue);
            String[] aliases = allAliases[i2];
            if (aliases == null) continue;
            for (String alias : aliases) {
                if (map2.containsKey(alias)) continue;
    ...
    protected static EnumResolver _constructUsingMethod(Class<?> enumCls0, AnnotatedMember accessor, AnnotationIntrospector ai, boolean isIgnoreCase) {
        ...
        return new EnumResolver(enumCls, enumConstants, map2, EnumResolver._enumDefault(ai, enumCls), isIgnoreCase, EnumResolver._isIntType(accessor.getRawType()));
    }
    ...
    @Deprecated
    protected EnumResolver(Class<Enum<?>> enumClass, Enum<?>[] enums, HashMap<String, Enum<?>> map2, Enum<?> defaultValue) {
        this(enumClass, enums, map2, defaultValue, false, false);
    }
