...
implements Serializable {
    ...
    protected DeserializationContext(DeserializerFactory df, DeserializerCache cache2) {
        if (df == null) {
            throw new NullPointerException("Cannot pass null DeserializerFactory");
        }
        this._factory = df;
        if (cache2 == null) {
            cache2 = new DeserializerCache();
        }
        this._cache = cache2;
    ...
    public <T> T readTreeAsValue(JsonNode n2, Class<T> targetType) throws IOException {
        if (n2 == null) {
            return null;
        }
        try (TreeTraversingParser p = this._treeAsTokens(n2);){
            T t = this.readValue((JsonParser)p, targetType);
            return t;
        }
    }

    public <T> T readTreeAsValue(JsonNode n2, JavaType targetType) throws IOException {
        if (n2 == null) {
            return null;
        }
        try (TreeTraversingParser p = this._treeAsTokens(n2);){
    ...
    private TreeTraversingParser _treeAsTokens(JsonNode n2) throws IOException {
        ObjectCodec codec = this._parser == null ? null : this._parser.getCodec();
        TreeTraversingParser p = new TreeTraversingParser(n2, codec);
        p.nextToken();
        return p;
    }
    ...
    public boolean handleUnknownProperty(JsonParser p, JsonDeserializer<?> deser, Object instanceOrClass, String propName) throws IOException {
        for (LinkedNode<DeserializationProblemHandler> h2 = this._config.getProblemHandlers(); h2 != null; h2 = h2.next()) {
            if (!h2.value().handleUnknownProperty(this, p, deser, instanceOrClass, propName)) continue;
            return true;
        }
    ...
    public Object handleWeirdNativeValue(JavaType targetType, Object badValue, JsonParser p) throws IOException {
        ...
        for (LinkedNode<DeserializationProblemHandler> h2 = this._config.getProblemHandlers(); h2 != null; h2 = h2.next()) {
            Object goodValue = h2.value().handleWeirdNativeValue(this, targetType, badValue, p);
    ...
    public Object handleMissingInstantiator(Class<?> instClass, ValueInstantiator valueInst, JsonParser p, String msg, Object ... msgArgs) throws IOException {
        ...
        msg = this._format(msg, msgArgs);
        for (LinkedNode<DeserializationProblemHandler> h2 = this._config.getProblemHandlers(); h2 != null; h2 = h2.next()) {
            Object instance = h2.value().handleMissingInstantiator(this, instClass, valueInst, p, msg);
            if (instance == DeserializationProblemHandler.NOT_HANDLED) continue;
    ...
    public Object handleInstantiationProblem(Class<?> instClass, Object argument, Throwable t) throws IOException {
        for (LinkedNode<DeserializationProblemHandler> h2 = this._config.getProblemHandlers(); h2 != null; h2 = h2.next()) {
            Object instance = h2.value().handleInstantiationProblem(this, instClass, argument, t);
            if (instance == DeserializationProblemHandler.NOT_HANDLED) continue;
            if (this._isCompatible(instClass, instance)) {
                return instance;
            }
    ...
    public Object handleUnexpectedToken(JavaType targetType, JsonToken t, JsonParser p, String msg, Object ... msgArgs) throws IOException {
        msg = this._format(msg, msgArgs);
        for (LinkedNode<DeserializationProblemHandler> h2 = this._config.getProblemHandlers(); h2 != null; h2 = h2.next()) {
            Object instance = h2.value().handleUnexpectedToken(this, targetType, t, p, msg);
            if (instance == DeserializationProblemHandler.NOT_HANDLED) continue;
    ...
    public <T> T reportBadCoercion(JsonDeserializer<?> src, Class<?> targetType, Object inputValue, String msg, Object ... msgArgs) throws JsonMappingException {
        msg = this._format(msg, msgArgs);
        InvalidFormatException e2 = InvalidFormatException.from(this.getParser(), msg, inputValue, targetType);
        throw e2;
    }
    ...
    @Deprecated
    public JsonMappingException mappingException(Class<?> targetClass, JsonToken token2) {
        return JsonMappingException.from(this._parser, String.format("Cannot deserialize instance of %s out of %s token", new Object[]{ClassUtil.nameOf(targetClass), token2}));
    }
