...
extends ParserMinimalBase {
    ...
    protected static final JacksonFeatureSet<StreamReadCapability> CBOR_READ_CAPABILITIES = DEFAULT_READ_CAPABILITIES.with(StreamReadCapability.EXACT_FLOATS);
                ...
                if (lowBits <= 23) {
                    ...
                    switch (lowBits - 24) {
                        ...
                        case 2: {
                            ...
                            long l2 = v;
                            this._numberLong = l2 & 0xFFFFFFFFL;
                            this._numTypesValid = 2;
                            break;
                        }
                        case 3: {
                            long l3 = this._decode64Bits();
                            if (l3 >= 0L) {
                                this._numberLong = l3;
                                this._numTypesValid = 2;
                                break;
                            }
                            this._numberBigInt = this._bigPositive(l3);
            ...
            case 1: {
                ...
                if (lowBits <= 23) {
                    ...
                    switch (lowBits - 24) {
                        ...
                        case 3: {
                            long l4 = this._decode64Bits();
                            if (l4 >= 0L) {
                                this._numberLong = -l4 - 1L;
                                this._numTypesValid = 2;
                                break;
                            }
                            this._numberBigInt = this._bigNegative(l4);
                            this._numTypesValid = 4;
                            break;
                        }
                        default: {
    ...
    protected String _numberToName(int ch, boolean neg) throws IOException {
        ...
        if (lowBits <= 23) {
            ...
            switch (lowBits) {
                case 24: {
                    i2 = this._decode8Bits();
                    break;
                }
                case 25: {
                    i2 = this._decode16Bits();
                    break;
                }
                case 26: {
                    long l2;
                    i2 = this._decode32Bits();
                    if (i2 >= 0) break;
                    if (neg) {
                        long unsignedBase = (long)i2 & 0xFFFFFFFFL;
                        l2 = -unsignedBase - 1L;
                    } else {
                        l2 = i2;
                        l2 &= 0xFFFFFFFFL;
        ...
        if (this._streamReadContext.inObject() && this._currToken != JsonToken.FIELD_NAME) {
            ...
            if (ptr + byteLen + 1 < this._inputEnd && ((ch = this._inputBuffer[ptr++]) >> 5 & 7) == 3 && (lenMarker = ch & 0x1F) <= 24) {
                ...
                if (lenMarker == byteLen) {
                    ...
                    while (true) {
                        if (i2 == lenMarker) {
                            this._inputPtr = ptr + i2;
                            this._streamReadContext.setCurrentName(str.getValue());
                            this._currToken = JsonToken.FIELD_NAME;
                            return true;
                        }
                        if (nameBytes[i2] != this._inputBuffer[ptr + i2]) break;
                        ++i2;
                    }
                }
            }
    ...
    public int getText(Writer writer) throws IOException {
        ...
        if (t == JsonToken.FIELD_NAME) {
            String n2 = this._streamReadContext.getCurrentName();
            writer.write(n2);
            return n2.length();
        }
        if (t != null) {
    ...
    public int readBinaryValue(Base64Variant b64variant, OutputStream out) throws IOException {
        if (this._currToken != JsonToken.VALUE_EMBEDDED_OBJECT) {
            if (this._currToken == JsonToken.VALUE_STRING) {
                byte[] b2 = this._getBinaryFromString(b64variant);
                int len = b2.length;
                out.write(b2, 0, len);
                return len;
            }
    ...
    private int _readAndWriteBytes(OutputStream out, int total) throws IOException {
        int count2;
        for (int left = total; left > 0; left -= count2) {
            ...
            count2 = Math.min(avail, left);
            out.write(this._inputBuffer, this._inputPtr, count2);
            this._inputPtr += count2;
        }
        this._tokenIncomplete = false;
        return total;
    }
        ...
        do {
            ...
            switch (codes[i2]) {
                ...
                case 2: {
                    ...
                    i2 = (i2 & 0xF) << 12 | (c2 & 0x3F) << 6 | c3 & 0x3F;
                    break;
                }
                case 3: {
                    i2 = (i2 & 7) << 18 | (inputBuf[inPtr++] & 0x3F) << 12 | (inputBuf[inPtr++] & 0x3F) << 6 | inputBuf[inPtr++] & 0x3F;
                    outBuf[outPtr++] = (char)(0xD800 | (i2 -= 65536) >> 10);
                    i2 = 0xDC00 | i2 & 0x3FF;
                    break;
                }
                default: {
    ...
    private final void _finishChunkedText() throws IOException {
        ...
        while (true) {
            ...
            if ((code = codes[c2 = input[this._inputPtr++] & 0xFF]) == 0 && outPtr < outEnd) {
                outBuf[outPtr++] = (char)c2;
                continue;
            }
            switch (code) {
                ...
                case 2: {
                    c2 = this._decodeChunkedUTF8_3(c2);
                    break;
                }
                case 3: {
                    c2 = this._decodeChunkedUTF8_4(c2);
                    if (outPtr >= outBuf.length) {
                        outBuf = this._textBuffer.finishCurrentSegment();
                        outPtr = 0;
                        outEnd = outBuf.length;
    ...
    protected byte[] _finishBytes(int len) throws IOException {
        ...
        while (true) {
            ...
            System.arraycopy(this._inputBuffer, this._inputPtr, b2, ptr, toAdd);
    ...
    protected byte[] _finishChunkedBytes() throws IOException {
        ...
        block0: while (true) {
            ...
            while (true) {
                ...
                int count2 = Math.min(avail, len);
                bb.write(this._inputBuffer, this._inputPtr, count2);
                this._inputPtr += count2;
                len -= count2;
            }
            break;
    ...
    protected byte[] _finishLongContiguousBytes(int expLen) throws IOException {
        try (ByteArrayBuilder bb = new ByteArrayBuilder(125000);){
            int count2;
            for (int left = expLen; left > 0; left -= count2) {
                int avail = this._inputEnd - this._inputPtr;
                if (avail <= 0) {
                    if (!this.loadMore()) {
    ...
    private final String _decodeContiguousName(int len) throws IOException {
        ...
        int end = inPtr + len;
        while ((code = codes[i2 = inBuf[inPtr] & 0xFF]) == 0) {
            outBuf[outPtr++] = (char)i2;
            if (++inPtr != end) continue;
            return this._textBuffer.setCurrentAndReturn(outPtr);
        }
        while (inPtr < end) {
            if ((code = codes[i2 = inBuf[inPtr++] & 0xFF]) != 0) {
                if (inPtr + code > end) {
                    int firstCharOffset = len - (end - inPtr) - 1;
                    this._reportTruncatedUTF8InName(len, firstCharOffset, i2, code);
                }
                switch (code) {
                    case 1: {
                        byte c2 = inBuf[inPtr++];
                    ...
                    case 3: {
                        i2 = (i2 & 7) << 18 | (inBuf[inPtr++] & 0x3F) << 12 | (inBuf[inPtr++] & 0x3F) << 6 | inBuf[inPtr++] & 0x3F;
                        outBuf[outPtr++] = (char)(0xD800 | (i2 -= 65536) >> 10);
                        i2 = 0xDC00 | i2 & 0x3FF;
                        break;
                    }
                    default: {
                        throw this._constructReadException("Invalid UTF-8 byte 0x%s in Object property name", Integer.toHexString(i2));
                    }
                }
            }
            outBuf[outPtr++] = (char)i2;
    ...
    protected final void _decodeNonStringName(int ch) throws IOException {
        ...
        if (type == 0) {
            ...
            byte[] b2 = this._finishBytes(blen);
            name = new String(b2, UTF8);
        } else {
            if ((ch & 0xFF) == 255) {
                this._reportUnexpectedBreak();
            }
    ...
    private final String _findDecodedFromSymbols(int len) throws IOException {
        if (len < 5) {
            int inPtr = this._inputPtr;
            byte[] inBuf = this._inputBuffer;
            int q = CBORParser._padQuadForNulls(inBuf[inPtr]);
            if (len > 1) {
        ...
        q1 = q1 << 8 | inBuf[inPtr++] & 0xFF;
        if (len < 9) {
            int q2 = CBORParser._padQuadForNulls(inBuf[inPtr++]);
            int left = len - 5;
            if (left > 0) {
        ...
        q2 = q2 << 8 | inBuf[inPtr++] & 0xFF;
        if (len < 13) {
            int q3 = CBORParser._padQuadForNulls(inBuf[inPtr++]);
            int left = len - 9;
            if (left > 0) {
    ...
    private final String _findDecodedLong(int len, int q1, int q2) throws IOException {
        ...
        if (len > 0) {
            q = CBORParser._padQuadForNulls(inBuf[inPtr]);
            if (len > 1) {
                q = (q << 8) + (inBuf[++inPtr] & 0xFF);
                if (len > 2) {
                    q = (q << 8) + (inBuf[++inPtr] & 0xFF);
    ...
    private final int _decodeTag(int lowBits) throws IOException {
        ...
        switch (lowBits - 24) {
            ...
            case 3: {
                long l2 = this._decode64Bits();
                if (l2 < Integer.MIN_VALUE || l2 > Integer.MAX_VALUE) {
                    throw this._constructReadException("Illegal Tag value: %d", l2);
                }
                return (int)l2;
            }
        }
        ...
        switch (lowBits - 24) {
            ...
            case 3: {
                ...
                if (l2 < 0L || l2 > Integer.MAX_VALUE) {
                    throw this._constructError("Illegal length for " + (Object)((Object)this.currentToken()) + ": " + l2);
                }
                return (int)l2;
            }
        }
    ...
    private float _decodeHalfSizeFloat() throws IOException {
        ...
        int f2 = i16 & 0x3FF;
        if (e2 == 0) {
            float result2 = (float)(MATH_POW_2_NEG14 * ((double)f2 / MATH_POW_2_10));
        ...
        float result3 = (float)(Math.pow(2.0, e2 - 15) * (1.0 + (double)f2 / MATH_POW_2_10));
        return neg ? -result3 : result3;
    }
    ...
    private final int _decode16Bits() throws IOException {
        ...
        byte[] b2 = this._inputBuffer;
        int v = ((b2[ptr] & 0xFF) << 8) + (b2[ptr + 1] & 0xFF);
    ...
    private final int _decode32Bits() throws IOException {
        ...
        byte[] b2 = this._inputBuffer;
        int v = (b2[ptr++] << 24) + ((b2[ptr++] & 0xFF) << 16) + ((b2[ptr++] & 0xFF) << 8) + (b2[ptr++] & 0xFF);
        this._inputPtr = ptr;
        return v;
    }
    ...
    private final long _decode64Bits() throws IOException {
        ...
        byte[] b2 = this._inputBuffer;
        int i1 = (b2[ptr++] << 24) + ((b2[ptr++] & 0xFF) << 16) + ((b2[ptr++] & 0xFF) << 8) + (b2[ptr++] & 0xFF);
        int i2 = (b2[ptr++] << 24) + ((b2[ptr++] & 0xFF) << 16) + ((b2[ptr++] & 0xFF) << 8) + (b2[ptr++] & 0xFF);
        this._inputPtr = ptr;
    ...
    private final int _decodeChunkedUTF8_4(int c2) throws IOException {
        int d2 = this._nextChunkedByte();
        if ((d2 & 0xC0) != 128) {
            this._reportInvalidOther(d2 & 0xFF, this._inputPtr);
        }
    ...
    private final BigInteger _bigPositive(long l2) {
        BigInteger biggie = BigInteger.valueOf(l2 << 1 >>> 1);
        return biggie.or(BIT_63);
    }

    private final BigInteger _bigNegative(long l2) {
        BigInteger unsignedBase = this._bigPositive(l2);
