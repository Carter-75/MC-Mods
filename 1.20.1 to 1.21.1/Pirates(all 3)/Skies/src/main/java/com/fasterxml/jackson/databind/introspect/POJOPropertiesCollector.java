...
public class POJOPropertiesCollector {
    ...
    protected void _addFields(Map<String, POJOPropertyBuilder> props) {
        ...
        for (AnnotatedField f2 : this._classDef.fields()) {
            ...
            if (Boolean.TRUE.equals(ai.hasAsKey(this._config, f2))) {
                if (this._jsonKeyAccessors == null) {
                    this._jsonKeyAccessors = new LinkedList();
                }
                this._jsonKeyAccessors.add(f2);
            }
            if (Boolean.TRUE.equals(ai.hasAsValue(f2))) {
                if (this._jsonValueAccessors == null) {
            ...
            boolean anyGetter = Boolean.TRUE.equals(ai.hasAnyGetter(f2));
            boolean anySetter = Boolean.TRUE.equals(ai.hasAnySetter(f2));
            if (anyGetter || anySetter) {
                if (anyGetter) {
                    if (this._anyGetterField == null) {
                        this._anyGetterField = new LinkedList();
                    }
                    this._anyGetterField.add(f2);
                }
            ...
            String implName = ai.findImplicitPropertyName(f2);
            if (implName == null) {
                implName = f2.getName();
            }
            if ((implName = this._accessorNaming.modifyFieldName(f2, implName)) == null) continue;
            PropertyName implNameP = this._propNameFromSimple(implName);
            PropertyName rename = ai.findRenameByField(this._config, f2, implNameP);
            ...
            if ((nameExplicit = (hasName = (pn = this._forSerialization ? ai.findNameForSerialization(f2) : ai.findNameForDeserialization(f2)) != null)) && pn.isEmpty()) {
            ...
            boolean ignored = ai.hasIgnoreMarker(f2);
    ...
    protected void _addMethods(Map<String, POJOPropertyBuilder> props) {
        for (AnnotatedMethod m2 : this._classDef.memberMethods()) {
            ...
            if (argCount == 0) {
                this._addGetterMethod(props, m2, this._annotationIntrospector);
                continue;
            }
            if (argCount == 1) {
                this._addSetterMethod(props, m2, this._annotationIntrospector);
                continue;
            }
            if (argCount != 2 || !Boolean.TRUE.equals(this._annotationIntrospector.hasAnySetter(m2))) continue;
            if (this._anySetters == null) {
    ...
    protected void _addGetterMethod(Map<String, POJOPropertyBuilder> props, AnnotatedMethod m2, AnnotationIntrospector ai) {
        boolean visible;
        String implName;
        boolean nameExplicit;
        Class<?> rt = m2.getRawReturnType();
        if (rt == Void.TYPE || rt == Void.class && !this._config.isEnabled(MapperFeature.ALLOW_VOID_VALUED_PROPERTIES)) {
            return;
        }
        if (Boolean.TRUE.equals(ai.hasAnyGetter(m2))) {
            if (this._anyGetters == null) {
        ...
        if (Boolean.TRUE.equals(ai.hasAsKey(this._config, m2))) {
            if (this._jsonKeyAccessors == null) {
                this._jsonKeyAccessors = new LinkedList();
            }
            this._jsonKeyAccessors.add(m2);
            return;
        }
        if (Boolean.TRUE.equals(ai.hasAsValue(m2))) {
            if (this._jsonValueAccessors == null) {
                this._jsonValueAccessors = new LinkedList();
            }
        ...
        PropertyName pn = ai.findNameForSerialization(m2);
        boolean bl = nameExplicit = pn != null;
        if (!nameExplicit) {
            implName = ai.findImplicitPropertyName(m2);
            if (implName == null) {
                implName = this._accessorNaming.findNameForRegularGetter(m2, m2.getName());
            }
            if (implName == null) {
                implName = this._accessorNaming.findNameForIsGetter(m2, m2.getName());
                if (implName == null) {
                    return;
                }
            ...
            implName = ai.findImplicitPropertyName(m2);
            if (implName == null && (implName = this._accessorNaming.findNameForRegularGetter(m2, m2.getName())) == null) {
                implName = this._accessorNaming.findNameForIsGetter(m2, m2.getName());
            }
            if (implName == null) {
                implName = m2.getName();
        ...
        boolean ignore = ai.hasIgnoreMarker(m2);
        this._property(props, implName).addGetter(m2, pn, nameExplicit, visible, ignore);
    }

    protected void _addSetterMethod(Map<String, POJOPropertyBuilder> props, AnnotatedMethod m2, AnnotationIntrospector ai) {
        boolean visible;
        String implName;
        boolean nameExplicit;
        PropertyName pn = ai.findNameForDeserialization(m2);
        boolean bl = nameExplicit = pn != null;
        if (!nameExplicit) {
            implName = ai.findImplicitPropertyName(m2);
            if (implName == null) {
                implName = this._accessorNaming.findNameForMutator(m2, m2.getName());
            }
            if (implName == null) {
                return;
            }
    ...
    protected void _addInjectables(Map<String, POJOPropertyBuilder> props) {
        for (AnnotatedField f2 : this._classDef.fields()) {
            this._doAddInjectable(this._annotationIntrospector.findInjectableValue(f2), f2);
        }
        for (AnnotatedMethod m2 : this._classDef.memberMethods()) {
            if (m2.getParameterCount() != 1) continue;
            this._doAddInjectable(this._annotationIntrospector.findInjectableValue(m2), m2);
        }
    }

    protected void _doAddInjectable(JacksonInject.Value injectable, AnnotatedMember m2) {
        AnnotatedMember prev2;
        if (injectable == null) {
            return;
        }
        ...
        if ((prev2 = this._injectables.put(id, m2)) != null && prev2.getClass() == m2.getClass()) {
