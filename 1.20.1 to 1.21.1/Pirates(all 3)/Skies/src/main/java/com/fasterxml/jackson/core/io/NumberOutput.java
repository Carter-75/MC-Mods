...
public final class NumberOutput {
    ...
    public static int outputInt(int v, char[] b2, int off) {
        if (v < 0) {
        ...
        if (v < MILLION) {
            if (v < 1000) {
                ...
                return NumberOutput._leading3(v, b2, off);
            }
            int thousands = v / 1000;
            off = NumberOutput._leading3(thousands, b2, off);
            off = NumberOutput._full3(v -= thousands * 1000, b2, off);
            return off;
        }
        if (v >= BILLION) {
            ...
            return NumberOutput._outputFullBillion(v, b2, off);
        }
        int newValue = v / 1000;
    ...
    public static int outputInt(int v, byte[] b2, int off) {
        if (v < 0) {
            if (v == Integer.MIN_VALUE) {
                return NumberOutput._outputSmallestI(b2, off);
            }
        ...
        if (v < MILLION) {
            if (v < 1000) {
                if (v < 10) {
                    ...
                    off = NumberOutput._leading3(v, b2, off);
                }
            } else {
                int thousands = v / 1000;
                off = NumberOutput._leading3(thousands, b2, off);
    ...
    public static int outputLong(long v, char[] b2, int off) {
        if (v < 0L) {
            if (v > MIN_INT_AS_LONG) {
                return NumberOutput.outputInt((int)v, b2, off);
            }
            if (v == Long.MIN_VALUE) {
        ...
        if (upper < BILLION_L) {
            ...
            off = NumberOutput._leading3((int)hi, b2, off);
            off = NumberOutput._outputFullBillion((int)(upper -= hi * BILLION_L), b2, off);
        }
        return NumberOutput._outputFullBillion((int)v, b2, off);
    }

    public static int outputLong(long v, byte[] b2, int off) {
        if (v < 0L) {
            if (v > MIN_INT_AS_LONG) {
                return NumberOutput.outputInt((int)v, b2, off);
            }
    ...
    private static int _outputUptoBillion(int v, char[] b2, int off) {
        if (v < MILLION) {
            if (v < 1000) {
                return NumberOutput._leading3(v, b2, off);
            }
    ...
    private static int _outputFullBillion(int v, char[] b2, int off) {
        int thousands = v / 1000;
        int ones = v - thousands * 1000;
        int millions = thousands / 1000;
    ...
    private static int _outputUptoBillion(int v, byte[] b2, int off) {
        if (v < MILLION) {
            if (v < 1000) {
                return NumberOutput._leading3(v, b2, off);
            }
    ...
    private static int _outputFullBillion(int v, byte[] b2, int off) {
        int thousands = v / 1000;
        int ones = v - thousands * 1000;
        int millions = thousands / 1000;
        int enc = TRIPLET_TO_CHARS[millions];
    ...
    private static int _leading3(int t, char[] b2, int off) {
    ...
    private static int _leading3(int t, byte[] b2, int off) {
        int enc = TRIPLET_TO_CHARS[t];
        if (t > 9) {
            if (t > 99) {
                b2[off++] = (byte)(enc >> 16);
    ...
    private static int _full3(int t, char[] b2, int off) {
        int enc = TRIPLET_TO_CHARS[t];
        b2[off++] = (char)(enc >> 16);
        b2[off++] = (char)(enc >> 8 & 0x7F);
        b2[off++] = (char)(enc & 0x7F);
    ...
    private static int _full3(int t, byte[] b2, int off) {
        int enc = TRIPLET_TO_CHARS[t];
        b2[off++] = (byte)(enc >> 16);
        b2[off++] = (byte)(enc >> 8);
        b2[off++] = (byte)enc;
