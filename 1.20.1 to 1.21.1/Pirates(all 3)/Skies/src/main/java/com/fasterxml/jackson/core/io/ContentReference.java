...
implements Serializable {
    ...
    protected String _truncate(byte[] b2, int[] offsets, int maxSnippetLen) {
        this._truncateOffsets(offsets, b2.length);
        int start = offsets[0];
        int length = Math.min(offsets[1], maxSnippetLen);
        return new String(b2, start, length, Charset.forName("UTF-8"));
    }
