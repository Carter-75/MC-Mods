...
extends NonBlockingJsonParserBase {
    ...
    protected JsonToken _finishErrorToken() throws IOException {
        while (this._inputPtr < this._inputEnd) {
            byte i2 = this.getNextSignedByteFromBuffer();
            char ch = (char)i2;
    ...
    protected JsonToken _finishNumberPlusMinus(int ch, boolean negative) throws IOException {
        ...
        if (ch <= 48) {
            ...
            if (ch == 73) {
                int token2 = negative ? 3 : 2;
                return this._finishNonStdToken(token2, 2);
            }
            message = negative ? "expected digit (0-9) to follow minus sign, for valid numeric value" : "expected digit (0-9) for valid numeric value";
            this._reportUnexpectedNumberChar(ch, message);
        }
    ...
    private final String _fastParseName() throws IOException {
        ...
        if (codes[q0 = this.getByteFromBuffer(ptr++) & 0xFF] == 0) {
            int i2;
            if (codes[i2 = this.getByteFromBuffer(ptr++) & 0xFF] == 0) {
                int q = q0 << 8 | i2;
                if (codes[i2 = this.getByteFromBuffer(ptr++) & 0xFF] == 0) {
                    q = q << 8 | i2;
                    if (codes[i2 = this.getByteFromBuffer(ptr++) & 0xFF] == 0) {
    ...
    private final String _parseMediumName2(int ptr, int q3, int q2) throws IOException {
        ...
        q3 = q3 << 8 | i2;
        if ((i2 = this.getByteFromBuffer(ptr++) & 0xFF) == 34) {
            this._inputPtr = ptr;
    ...
    private int _decodeSplitEscaped(int value, int bytesRead) throws IOException {
        ...
        int c2 = this.getNextSignedByteFromBuffer();
        if (bytesRead == -1) {
            switch (c2) {
                case 98: {
                    return 8;
            ...
            c2 = this.getNextSignedByteFromBuffer();
            bytesRead = 0;
        }
        c2 &= 0xFF;
        while (true) {
            ...
            c2 = this.getNextUnsignedByteFromBuffer();
        }
    }
    ...
    protected JsonToken _startString() throws IOException {
        ...
        for (ptr = this._inputPtr; ptr < max; ++ptr) {
            int c2 = this.getByteFromBuffer(ptr) & 0xFF;
            if (codes[c2] != 0) {
                if (c2 != 34) break;
                this._inputPtr = ptr + 1;
                this._textBuffer.setCurrentLength(outPtr);
    ...
    private final JsonToken _finishRegularString() throws IOException {
        ...
        block6: while (true) {
            ...
            while (true) {
                ...
                if (codes[c2 = this.getByteFromBuffer(ptr++) & 0xFF] != 0) break;
                outBuf[outPtr++] = (char)c2;
            }
            if (c2 == 34) {
                this._inputPtr = ptr;
            ...
            switch (codes[c2]) {
                ...
                case 2: {
                    c2 = this._decodeUTF8_2(c2, this.getByteFromBuffer(ptr++));
                    break;
                }
                case 3: {
                    c2 = this._decodeUTF8_3(c2, this.getByteFromBuffer(ptr++), this.getByteFromBuffer(ptr++));
                    break;
                }
                case 4: {
                    c2 = this._decodeUTF8_4(c2, this.getByteFromBuffer(ptr++), this.getByteFromBuffer(ptr++), this.getByteFromBuffer(ptr++));
    ...
    private final JsonToken _finishAposString() throws IOException {
        ...
        block6: while (true) {
            ...
            while (true) {
                if (ptr >= max) continue block6;
                if (codes[c2 = this.getByteFromBuffer(ptr++) & 0xFF] != 0 && c2 != 34) break;
                if (c2 == 39) {
                    this._inputPtr = ptr;
                    this._textBuffer.setCurrentLength(outPtr);
                    return this._valueComplete(JsonToken.VALUE_STRING);
    ...
    private final boolean _decodeSplitMultiByte(int c2, int type, boolean gotNext) throws IOException {
        switch (type) {
            ...
            case 2: {
                if (gotNext) {
                    c2 = this._decodeUTF8_2(c2, this.getNextSignedByteFromBuffer());
                    this._textBuffer.append((char)c2);
                    return true;
                }
                this._minorState = 42;
            ...
            case 3: {
                c2 &= 0xF;
                if (gotNext) {
                    return this._decodeSplitUTF8_3(c2, 1, this.getNextSignedByteFromBuffer());
                }
                this._minorState = 43;
                this._pending32 = c2;
            ...
            case 4: {
                c2 &= 7;
                if (gotNext) {
                    return this._decodeSplitUTF8_4(c2, 1, this.getNextSignedByteFromBuffer());
                }
                this._pending32 = c2;
                this._pendingBytes = 1;
                this._minorState = 44;
    ...
    private final boolean _decodeSplitUTF8_3(int prev2, int prevCount, int next) throws IOException {
        if (prevCount == 1) {
            if ((next & 0xC0) != 128) {
                this._reportInvalidOther(next & 0xFF, this._inputPtr);
            }
            prev2 = prev2 << 6 | next & 0x3F;
            if (this._inputPtr >= this._inputEnd) {
                this._minorState = 43;
                this._pending32 = prev2;
        ...
        this._textBuffer.append((char)(prev2 << 6 | next & 0x3F));
        return true;
    }

    private final boolean _decodeSplitUTF8_4(int prev2, int prevCount, int next) throws IOException {
        if (prevCount == 1) {
            if ((next & 0xC0) != 128) {
                this._reportInvalidOther(next & 0xFF, this._inputPtr);
            }
        ...
        int c2 = (prev2 << 6 | next & 0x3F) - 65536;
    ...
    private final int _decodeFastCharEscape() throws IOException {
        int digit;
        byte c2 = this.getNextSignedByteFromBuffer();
        switch (c2) {
            case 98: {
                return 8;
            }
