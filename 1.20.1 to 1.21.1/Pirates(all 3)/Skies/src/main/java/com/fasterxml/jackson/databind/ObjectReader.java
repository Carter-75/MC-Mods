...
Serializable {
    ...
    protected ObjectReader(ObjectReader base, TokenFilter filter2) {
    ...
    public ObjectReader with(DeserializationFeature first2, DeserializationFeature ... other) {
        return this._with(this._config.with(first2, other));
    }
    ...
    public ObjectReader without(DeserializationFeature first2, DeserializationFeature ... other) {
        return this._with(this._config.without(first2, other));
    }
    ...
    public ObjectReader with(JsonFactory f2) {
        ...
        ObjectReader r = this._new(this, f2);
        if (f2.getCodec() == null) {
            f2.setCodec(r);
        }
        return r;
    ...
    public boolean isEnabled(DatatypeFeature f2) {
        return this._config.isEnabled(f2);
    }

    public boolean isEnabled(JsonParser.Feature f2) {
        return this._config.isEnabled(f2, this._parserFactory);
    }

    public boolean isEnabled(StreamReadFeature f2) {
        return this._config.isEnabled(f2.mappedFeature(), this._parserFactory);
    }
    ...
    public <T> T treeToValue(TreeNode n2, JavaType valueType) throws JsonProcessingException {
        this._assertNotNull("n", n2);
        try {
            return this.readValue(this.treeAsTokens(n2), valueType);
        }
    ...
    protected Object _bind(JsonParser p, Object valueToUpdate) throws IOException {
        ...
        Object result2 = t == JsonToken.VALUE_NULL ? (valueToUpdate == null ? this._findRootDeserializer(ctxt).getNullValue(ctxt) : valueToUpdate) : (t == JsonToken.END_ARRAY || t == JsonToken.END_OBJECT ? valueToUpdate : ctxt.readRootValue(p, this._valueType, this._findRootDeserializer(ctxt), this._valueToUpdate));
        p.clearCurrentToken();
    ...
    protected Object _bindAndClose(JsonParser p0) throws IOException {
        try (JsonParser p = p0;){
            DefaultDeserializationContext ctxt = this.createDeserializationContext(p);
            JsonToken t = this._initForReading(ctxt, p);
            Object result2 = t == JsonToken.VALUE_NULL ? (this._valueToUpdate == null ? this._findRootDeserializer(ctxt).getNullValue(ctxt) : this._valueToUpdate) : (t == JsonToken.END_ARRAY || t == JsonToken.END_OBJECT ? this._valueToUpdate : ctxt.readRootValue(p, this._valueType, this._findRootDeserializer(ctxt), this._valueToUpdate));
            if (this._config.isEnabled(DeserializationFeature.FAIL_ON_TRAILING_TOKENS)) {
                this._verifyNoTrailingTokens(p, ctxt, this._valueType);
            }
