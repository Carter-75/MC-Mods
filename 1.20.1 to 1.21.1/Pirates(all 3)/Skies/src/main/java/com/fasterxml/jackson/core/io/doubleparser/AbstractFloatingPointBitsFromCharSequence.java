...
extends AbstractFloatValueParser {
    ...
    public long parseFloatingPointLiteral(CharSequence str, int offset, int length) {
        ...
        if (isNegative || ch == '+') {
            char c2 = ch = ++index < endIndex ? str.charAt(index) : (char)'\u0000';
            if (ch == '\u0000') {
                return -1L;
            }
        }
        ...
        if (hasLeadingZero) {
            char c3 = ch = ++index < endIndex ? str.charAt(index) : (char)'\u0000';
            if (ch == 'x' || ch == 'X') {
    ...
    private long parseHexFloatLiteral(CharSequence str, int index, int startIndex, int endIndex, boolean isNegative) {
        ...
        while (index < endIndex) {
            int hexValue;
            ch = str.charAt(index);
            int n2 = hexValue = ch > '\u007f' ? -1 : AbstractFloatValueParser.CHAR_TO_HEX_MAP[ch];
            if (hexValue >= 0) {
        ...
        if (digitCount > 16) {
            ...
            for (index = significandStartIndex; index < significandEndIndex; ++index) {
                int hexValue;
                ch = str.charAt(index);
                int n3 = hexValue = ch > '\u007f' ? -1 : AbstractFloatValueParser.CHAR_TO_HEX_MAP[ch];
                if (hexValue >= 0) {
                    if (Long.compareUnsigned(significand, 1000000000000000000L) >= 0) break;
                    significand = significand << 4 | (long)hexValue;
                    continue;
    ...
    private int tryToParseEightDigits(CharSequence str, int offset) {
        long first2 = (long)str.charAt(offset) | (long)str.charAt(offset + 1) << 16 | (long)str.charAt(offset + 2) << 32 | (long)str.charAt(offset + 3) << 48;
        long second2 = (long)str.charAt(offset + 4) | (long)str.charAt(offset + 5) << 16 | (long)str.charAt(offset + 6) << 32 | (long)str.charAt(offset + 7) << 48;
