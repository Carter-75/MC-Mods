...
extends CollectorBase {
    ...
    private List<AnnotatedMethod> _findPotentialFactories(TypeFactory typeFactory, JavaType type, Class<?> primaryMixIn) {
        ...
        for (Method m2 : ClassUtil.getClassMethods(type.getRawClass())) {
            if (!AnnotatedCreatorCollector._isIncludableFactoryMethod(m2)) continue;
            if (candidates == null) {
    ...
    private static boolean _isIncludableFactoryMethod(Method m2) {
        return Modifier.isStatic(m2.getModifiers()) && !m2.isSynthetic();
    }
    ...
    protected AnnotatedMethod constructFactoryCreator(Method m2, TypeResolutionContext typeResCtxt, Method mixin) {
        int paramCount = m2.getParameterCount();
        if (this._intr == null) {
            return new AnnotatedMethod(typeResCtxt, m2, AnnotatedCreatorCollector._emptyAnnotationMap(), AnnotatedCreatorCollector._emptyAnnotationMaps(paramCount));
        }
        if (paramCount == 0) {
            return new AnnotatedMethod(typeResCtxt, m2, this.collectAnnotations(m2, mixin), NO_ANNOTATION_MAPS);
        }
        return new AnnotatedMethod(typeResCtxt, m2, this.collectAnnotations(m2, mixin), this.collectAnnotations(m2.getParameterAnnotations(), mixin == null ? (Annotation[][])null : mixin.getParameterAnnotations()));
    }
