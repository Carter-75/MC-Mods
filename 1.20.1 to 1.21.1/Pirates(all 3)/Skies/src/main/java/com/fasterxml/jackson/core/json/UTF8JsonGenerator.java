...
extends JsonGeneratorImpl {
    ...
    public void writeNumber(double d2) throws IOException {
        ...
        this._verifyValueWrite("write a number");
        this.writeRaw(NumberOutput.toString(d2, this.isEnabled(JsonGenerator.Feature.USE_FAST_DOUBLE_WRITER)));
    }
    ...
    public void writeNumber(float f2) throws IOException {
        ...
        this._verifyValueWrite("write a number");
        this.writeRaw(NumberOutput.toString(f2, this.isEnabled(JsonGenerator.Feature.USE_FAST_DOUBLE_WRITER)));
    }
    ...
    private final int _readMore(InputStream in, byte[] readBuffer, int inputPtr, int inputEnd, int maxRead) throws IOException {
        ...
        while ((length = maxRead - inputEnd) != 0) {
            int count2 = in.read(readBuffer, inputEnd, length);
            if (count2 < 0) {
                return inputEnd;
            }
            if ((inputEnd += count2) < 3) continue;
        }
        return inputEnd;
    }
