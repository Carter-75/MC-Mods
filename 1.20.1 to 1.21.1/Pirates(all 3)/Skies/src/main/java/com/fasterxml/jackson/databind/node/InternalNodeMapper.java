...
final class InternalNodeMapper {
    ...
    public static String nodeToString(BaseJsonNode n2) {
        try {
            return STD_WRITER.writeValueAsString(InternalNodeMapper._wrapper(n2));
        }
    ...
    public static String nodeToPrettyString(BaseJsonNode n2) {
        try {
            return PRETTY_WRITER.writeValueAsString(InternalNodeMapper._wrapper(n2));
        }
        catch (IOException e2) {
            throw new RuntimeException(e2);
        }
    ...
    extends JsonSerializable.Base {
        ...
        @Override
        public void serialize(JsonGenerator g2, SerializerProvider ctxt) throws IOException {
            this._context = ctxt;
            this._serializeNonRecursive(g2, this._root);
        }

        @Override
        public void serializeWithType(JsonGenerator g2, SerializerProvider ctxt, TypeSerializer typeSer) throws IOException {
            this.serialize(g2, ctxt);
        }

        protected void _serializeNonRecursive(JsonGenerator g2, JsonNode node) throws IOException {
            if (node instanceof ObjectNode) {
                g2.writeStartObject(this, node.size());
                this._serializeNonRecursive(g2, new IteratorStack(), node.fields());
            } else if (node instanceof ArrayNode) {
                g2.writeStartArray(this, node.size());
                this._serializeNonRecursive(g2, new IteratorStack(), node.elements());
            } else {
                node.serialize(g2, this._context);
            }
        }

        protected void _serializeNonRecursive(JsonGenerator g2, IteratorStack stack, Iterator<?> rootIterator) throws IOException {
            ...
            while (true) {
                if (currIt.hasNext()) {
                    ...
                    if (elem instanceof Map.Entry) {
                        ...
                        g2.writeFieldName((String)en.getKey());
                    ...
                    if (value instanceof ObjectNode) {
                        stack.push(currIt);
                        currIt = value.fields();
                        g2.writeStartObject(value, value.size());
                        continue;
                    }
                    if (value instanceof ArrayNode) {
                        stack.push(currIt);
                        currIt = value.elements();
                        g2.writeStartArray(value, value.size());
                        continue;
                    }
                    value.serialize(g2, this._context);
                    continue;
                }
                if (g2.getOutputContext().inArray()) {
                    g2.writeEndArray();
                } else {
                    g2.writeEndObject();
                }
