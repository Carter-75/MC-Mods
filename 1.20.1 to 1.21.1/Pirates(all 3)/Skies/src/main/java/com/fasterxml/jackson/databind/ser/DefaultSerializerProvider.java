...
implements Serializable {
    ...
    public Object includeFilterInstance(BeanPropertyDefinition forProperty, Class<?> filterClass) {
        ...
        Object object = filter2 = hi == null ? null : hi.includeFilterInstance(this._config, forProperty, filterClass);
        if (filter2 == null) {
            filter2 = ClassUtil.createInstance(filterClass, this._config.canOverrideAccessModifiers());
        }
        return filter2;
    }

    @Override
    public boolean includeFilterSuppressNulls(Object filter2) throws JsonMappingException {
        if (filter2 == null) {
            return true;
        }
        try {
            return filter2.equals(null);
        }
        catch (Exception e2) {
            String msg = String.format("Problem determining whether filter of type '%s' should filter out `null` values: (%s) %s", filter2.getClass().getName(), e2.getClass().getName(), ClassUtil.exceptionMessage(e2));
            this.reportBadDefinition(filter2.getClass(), msg, (Throwable)e2);
            return false;
        }
    }
